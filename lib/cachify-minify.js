...
var crypto = require('crypto'),
    ...
    url = require('url'),
    compressor = require('node-minify-custom-fork');
...
var get_resource_hash = function (resource) {
    ...
    var req_url = url.parse(resource),
        uri = req_url.pathname || '';
    var cached = true;
    for (let url of _assets[uri]) {
        var assetFilename = url_to_filename(url);
        var assetData = fs.readFileSync(assetFilename);
        if (_cache[assetFilename] && _cache[assetFilename].hash == get_hash_for_contents(assetData)) {
            continue;
        }else{
            cached = false;
            break;
    ...
    if (cached) {
        if (!fs.existsSync(filename)) {
            if (_type == 'js') {
                compressor.minify({
                    ...
                    callback: function (err, min) {
                        try {
                            var mindata = fs.readFileSync(filename);
                            _cache[filename] = {exists: true};
                            _cache[filename].hash = get_hash_for_contents(mindata);
            ...
            if (_cache[filename] && _cache[filename].hash) {
                ...
                try {
                    var mindata = fs.readFileSync(filename);
                    _cache[filename] = {exists: true};
                    return _cache[filename].hash = get_hash_for_contents(mindata);
                } catch (e) {
                    exports.uncached_resources.push(resource);
                    console.error('Cachify bailing on hash... no such file ' + filename);
                    console.error('Options: %s', JSON.stringify(_opts));
        ...
        if (_type == 'js') {
            compressor.minify({
                ...
                output: resource,
                options: {
                    warnings: _opts.debug ? true : false, // pass true to display compressor warnings.
                    mangle: true // pass false to skip mangling names.
                },
                callback: function (err, min) {}
            });
            ...
            _cache[filename] = {exists: true};
            _cache[filename].hash = get_hash_for_contents(mindata);
            generate_hash_for_assets(_assets[uri]);
            return _cache[filename].hash;
        }
        if (_type == 'css') {
            compressor.minify({
...
var generate_hash_for_assets = function(array) {
    array.forEach(function (url) {
        var assetFilename = url_to_filename(url);
        var assetData = fs.readFileSync(assetFilename);
        _cache[assetFilename] = {exists: true};
        _cache[assetFilename].hash = get_hash_for_contents(assetData);
    });
};
...
var hashify = function (resource, hash) {
    ...
    hash = get_resource_hash(resource);
    if (!hash) {
        return resource;
    }
